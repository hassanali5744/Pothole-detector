...
function getStatusStyle(status: ReportStatus): string {
  ...
  return styleMap[status] || "bg-surface-muted text-ink-secondary ring-line";
}

function getSeverityStyle(severity: SeverityLevel): string {
  const styleMap: Record<SeverityLevel, string> = {
    low: "bg-surface-muted text-ink-secondary ring-line",
    medium: "bg-amber-50 text-amber-800 ring-amber-200",
    high: "bg-orange-50 text-orange-800 ring-orange-200",
    critical: "bg-danger-soft text-danger ring-red-200",
  };
  return styleMap[severity] || "bg-surface-muted text-ink-secondary ring-line";
}
...
export function StatusBadge({ status }: { status: ReportStatus }) {
  return (
    <Badge className={getStatusStyle(status)}>{STATUS_LABELS[status]}</Badge>
  );
}

export function SeverityBadge({ severity }: { severity: SeverityLevel }) {
  return (
    <Badge className={cn(getSeverityStyle(severity))}>
